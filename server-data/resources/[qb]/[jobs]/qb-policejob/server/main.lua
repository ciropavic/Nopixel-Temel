...
QBCore.Commands.Add("takedna", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) and OtherPlayer then
        if TriggerClientEvent("inventory:removeItem", src, "empty_evidence_bag", 1) then
            local info = {
                label = Lang:t('info.dna_sample'),
                type = "dna",
                dnalabel = DnaHash(OtherPlayer.PlayerData.citizenid)
            }
            if TriggerClientEvent("player:receiveItem", src, "filled_evidence_bag", 1, false, info) then
    ...
    if Player.Functions.GetItemByName(item.name) then
        if item.info and item.info ~= "" then
            if Player.PlayerData.job.name ~= "police" then
                if TriggerClientEvent("inventory:removeItem", src, "moneybag", 1, item.slot) then
...
RegisterNetEvent('police:server:SeizeCash', function(playerId)
    ...
    if SearchedPlayer then
        ...
        TriggerClientEvent("player:receiveItem", src, "moneybag", 1, false, info)
...
RegisterNetEvent('evidence:server:AddBlooddropToInventory', function(bloodId, bloodInfo)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if TriggerClientEvent("inventory:removeItem", src, "empty_evidence_bag", 1) then
        if TriggerClientEvent("player:receiveItem", src, "filled_evidence_bag", 1, false, bloodInfo) then
            TriggerClientEvent("inventory:client:ItemBox", src, QBCore.Shared.Items["filled_evidence_bag"], "add")
...
RegisterNetEvent('evidence:server:AddFingerprintToInventory', function(fingerId, fingerInfo)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if TriggerClientEvent("inventory:removeItem", src, "empty_evidence_bag", 1) then
        if TriggerClientEvent("player:receiveItem", src, "filled_evidence_bag", 1, false, fingerInfo) then
            TriggerClientEvent("inventory:client:ItemBox", src, QBCore.Shared.Items["filled_evidence_bag"], "add")
...
RegisterNetEvent('evidence:server:AddCasingToInventory', function(casingId, casingInfo)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if TriggerClientEvent("inventory:removeItem", src, "empty_evidence_bag", 1) then
        if TriggerClientEvent("player:receiveItem", src, "filled_evidence_bag", 1, false, casingInfo) then
            TriggerClientEvent("inventory:client:ItemBox", src, QBCore.Shared.Items["filled_evidence_bag"], "add")
...
AddEventHandler('server:evidenceBag:setNote', function(item,note)
    ...
    local i = item.info;
    i.evidenceNote=note;
    if(TriggerClientEvent("inventory:removeItem", src, "filled_evidence_bag",1,item.slot)) then
        TriggerClientEvent("player:receiveItem", src, "filled_evidence_bag",1,item.slot,i);
    end
end)
...
AddEventHandler("qb-spikes-use", function()
    ...
    TriggerClientEvent("inventory:removeItem", src, 'policespikes', Config.Amount, false, info) 
end)
...
AddEventHandler("qb-spikes-remove", function(netid)
    ...
    TriggerClientEvent("player:receiveItem", src, 'policespikes', 1, false, info) -- set to 1 still pulls the config spawned ammount...
...
RegisterServerEvent("qb-police:server:license", function(playerId, businessname, issuedate, expiredate)
    ...
    local info = {}
        ...
        TriggerClientEvent("player:receiveItem", src, 'certificate', 1, nil, info)
