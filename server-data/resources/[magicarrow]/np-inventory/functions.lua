...
AddEventHandler('police:openPerStash', function(percent)
    local QBCore = exports['qb-core']:GetCoreObject()
    local cid = QBCore.Functions.GetPlayerData().citizenid
...
AddEventHandler('police:openPDArmory', function(percent)
    TriggerEvent("server-inventory-open", "31", "Shop");
end)
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    ...
    if (itemid == "pd_dashcam") then
        TriggerEvent('rp_vehicles:client:GoPro:attach', source, item, 'pd')--Ventre
    end
    ...
    if (itemid == "harness") then
        local finished = exports["np-taskbar"]:taskBar(3000, "Kemeri Takıyorsun...", true, false)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
    ...
    if itemid == "cuffs" then
        TriggerEvent("police:client:CuffPlayerSoft", src)
    end

    if itemid == "cuffkey" then
        TriggerEvent("police:client:UnCuffPlayer", src)
    end
    ...
    if itemid == "radio" then
        TriggerEvent('qb-radio:use', source) --Ventre
    end
...
exports('hasEnoughOfItem', function(itemid,amount,shouldReturnText,checkQuality, metaInformation)
...
AddEventHandler('changethirst', function()
    local QBCore = exports['qb-core']:GetCoreObject()
    TriggerServerEvent("QBCore:Server:SetMetaData", "thirst", QBCore.Functions.GetPlayerData().metadata["thirst"] + 30)
end)
