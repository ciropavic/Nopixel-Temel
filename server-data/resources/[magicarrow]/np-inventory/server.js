        ...
        if (secondInventory == "1") {
            ...
            for (let Key in itemToDropArray) {
                for (let i = 0; i < itemToDropArray[Key].length; i++) {
                    ...
                    exports["oxmysql"].execute(`UPDATE player_inventory SET slot = '${i+1}', name = '${NewDroppedName}', dropped = '1' WHERE name = '${Key}' AND slot = '${objectToDrop.faultySlot}' AND item_id = '${objectToDrop.faultyItem}' `);
                }
            }
...
onNet("server-inventory-stack", async (cid, pData, playerPos) => {
    ...
    if (isPurchase) {
        if (isWeapon) {
            //exports["oxmysql"].execute(`INSERT INTO player_inventory (item_id, name, information, slot, creationDate) VALUES ('${itemId}','${targetInventory}','${info}','${targetSlot}','${creationDate}' );`);
        }
        ...
        if (PlayerStore) {
            exports["oxmysql"].execute(`UPDATE player_inventory SET slot = '${targetSlot}', name = '${targetInventory}', dropped = '0' WHERE slot = '${originSlot}' AND name = '${originInventory}'`);
        }
        ...
        exports["oxmysql"].execute(query, {}, function(result) {
            ...
            if (targetInventory.indexOf("Drop") > -1 || targetInventory.indexOf("hidden") > -1) {
                exports["oxmysql"].execute(`UPDATE player_inventory SET slot='${targetSlot}', name='${targetInventory}', dropped='1' WHERE id IN (${itemIds})`);

            } else {
                exports["oxmysql"].execute(`UPDATE player_inventory SET slot='${targetSlot}', name='${targetInventory}', dropped='0' WHERE id IN (${itemIds})`);
            }
        });
    }
});
...
onNet("server-inventory-move", async (cid, pData, playerPos) => {
    ...
    if (isPurchase) {
        ...
        if (!PlayerStore) {
            ...
            if (targetInventory.indexOf("Drop") > -1 || targetInventory.indexOf("hidden") > -1) {
                exports["oxmysql"].execute(`INSERT INTO player_inventory SET slot = '${targetSlot}', name = '${targetInventory}', dropped = '1' WHERE slot = '${originSlot}' AND name = '${originInventory}'`);
            } else {
                exports["oxmysql"].execute(`UPDATE player_inventory SET slot = '${targetSlot}', name = '${targetInventory}', dropped = '0' WHERE slot = '${originSlot}' AND name = '${originInventory}'`);
            }
        }
    ...
    exports["oxmysql"].execute(query, {}, function(result) {
        ...
        exports["oxmysql"].execute(query, {}, function(inventory) {
            if (originInventory.indexOf("Drop") > -1 || originInventory.indexOf("hidden") > -1) {
                exports["oxmysql"].execute(`UPDATE player_inventory SET slot = '${originSlot}', name = '${originInventory}', dropped = '1' WHERE id IN (${itemIds})`);
            } else {
                exports["oxmysql"].execute(`UPDATE player_inventory SET slot = '${originSlot}', name = '${originInventory}', dropped = '0' WHERE id IN (${itemIds})`);
            }
        });
    });
});
...
onNet("inventory:degItem", async (LastUsedItem, percent, LastUsedItemId, cid) => {
    ...
    exports["oxmysql"].execute(`UPDATE player_inventory SET creationDate = creationDate - ${amount} WHERE id = ${LastUsedItem} AND item_id = '${LastUsedItemId}' AND name = '${playerInventory}'`, {}, function() {});
});
...
function deleteHidden() {
    exports["oxmysql"].execute(`DELETE FROM player_inventory WHERE name like '%Hidden%' OR name like '%trash%'`)
}
...
onNet('onResourceStart', (resource) => {
    if (resource == GetCurrentResourceName()){
        exports["oxmysql"].execute(`DELETE FROM player_inventory WHERE name like '%Drop%'`)
    }
})
...
onNet("server-jail-item", async (player, isSentToJail) => {
    ...
    exports["oxmysql"].execute(`UPDATE player_inventory SET name='${currInventoryName}', WHERE name='${newInventoryName}' and dropped=0`);
});