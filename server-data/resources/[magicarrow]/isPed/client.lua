...
RegisterNetEvent('client:updategang', function(newGangType)
  gangType = newGangType
end)

RegisterNetEvent('incall', function(status)
  incall = status
end)


RegisterNetEvent("ped:intrunk", function(trunksent)
  intrunk = trunksent
  if intrunk then
...
function GetClosestPlayer()
  ...
  if not IsPedInAnyVehicle(PlayerPedId(), false) then
    ...
    TriggerEvent("notify","Inside Vehicle.",2)
  end
end
...
RegisterNetEvent('robbing',function(passedR)
    robbing = passedR
    if robbing then
      print("is robbing")
    else
...
RegisterNetEvent("returnPlayerKeys", function(ownedhouses,keyedhouses,keyhouseIDs)
    myhousekeys = keyhouseIDs
end)

RegisterNetEvent("enablegangmember", function(gangtype,cidpassed)
    gang = gangtype
    cid = cidpassed
end)

RegisterNetEvent("updatecid", function(cidpassed)
  cid = cidpassed
end)

RegisterNetEvent("phoneEnabled", function(phoneopensent)
 phoneopen = phoneopensent
end)

RegisterNetEvent("radioEnabled", function(radioopensent)
 radioopen = radioopensent
end)


RegisterNetEvent("updateNameClient", function(firstname,lastname)
  Firstname = firstname
  Lastname = lastname
end)
...
RegisterNetEvent('glxs:playerdead', function(bool)
	dead = bool
end)

RegisterNetEvent('police:currentHandCuffedState', function(handCuffed,handCuffedWalking)
	handcuffed = handCuffed
  handcuffedwalking = handCuffedWalking
end)

RegisterNetEvent('pagerStatus', function(PassedPagerStatus)
...
RegisterNetEvent('banking:updateBalance', function(amount)
...
RegisterNetEvent('isPed:UpdateCash', function(newCash)
  cash = newCash
end)
...
RegisterNetEvent('updateLicenseString', function(newLicenseString)
...
AddEventHandler('client:pass', function(passType,args)
    ...
    if rank < 1 then
      if passType == "strip_club" then
        TriggerEvent("notify","Hm, I wonder if I can join this club?",1)
      else
        TriggerEvent("notify","You do not have permission to do this.",2)
      end
      return
    end
    if passType == "golfcourse" then
      TriggerServerEvent("GolfPay")
      TriggerEvent("notify","Press E to swing, A-D to rotate, Y to swap club.",1)
    end
    if passType == "gym" then
      TriggerEvent("doworkout")
      TriggerEvent("notify","Starting, hold still..",1)
    end
    if passType == "strip_club" then
      TriggerEvent("dostripstress")
      TriggerEvent("notify","Ahhh, this is relaxing.",1)
    end    
